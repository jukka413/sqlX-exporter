...
// одним локом. revision присваивает app.reload() (см. app.revision), тем же
// значением, что и workerManager в этом же вызове.
...
// app_db_up по результату — Stats() (см. metricsUpdater) сообщает состояние
// пула соединений, а не то, отвечает ли сервер сейчас; без реального Ping
// тихо умершая сеть оставила бы app_db_up=1 бесконечно. Отдельная, более
// редкая горутина — Ping, в отличие от Stats(), реальный сетевой запрос.
