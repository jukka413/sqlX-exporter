...
Ключ — произвольное имя, используется в `queries.<name>.db`.
...
databases:
  main:
    ...
    health_check_period: "1m"    # зарезервировано, в текущей версии игнорируется
```
...
Ключ — имя запроса, становится именем метрики в Prometheus.
...
Вместо `interval` можно задать конкретные дни недели и время через `schedule` — полезно для тяжёлых отчётных запросов которые не нужно выполнять каждую минуту:
...
```dockerfile
ENV GOGC=50              # GC запускается вдвое чаще дефолта — меньше пиковый heap
ENV GOMEMLIMIT=55MiB     # мягкий лимит heap, держи чуть ниже resources.limits.memory
ENV GODEBUG=madvdontneed=1   # немедленно отдавать свободные страницы ОС (важно для RSS в k8s)
```
